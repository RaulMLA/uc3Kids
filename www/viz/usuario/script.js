...
visSocket.emit("GET_USUARIO");

visSocket.on("DAR_USUARIO", function(data) {
    document.querySelector(".nombre-usuario").innerHTML = data;
});; 

visSocket.emit("LOAD_USUARIO");
...
visSocket.on("UPDATE_CONTENT", function(html) {
    if (html != "") {
        ...
        document.querySelector(".contenedor").innerHTML = "<h1>No hay sugerencias</h1>";
    }
});
